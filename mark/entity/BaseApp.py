...
class BaseApp:
    #common-var
    user_info=None
    appdata_dir=os.path.join(os.getenv('localappdata'), 'ZeroEye', 'AnnotationTool')
    ...
    def get_conifgObj(filename='config.ini'):
        ...
        if not os.path.exists(file_path):
            # 先新建目录
            os.makedirs(os.path.join(os.getenv('localappdata'), 'ZeroEye', 'AnnotationTool', 'Logs'),exist_ok=1)
            # 初始化配置文件
