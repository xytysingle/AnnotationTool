...
def _shouldRelocateCommand(cmd):
    """
    Should this command sku_name be investigated for relocation?
    """
...
class MachO(object):
    ...
    #   id_cmd     - the index of my sku_name command, or None
        ...
        for i in range(header.ncmds):
            ...
            if cmd_load.cmd == LC_ID_DYLIB:
                ...
                if self.id_cmd is not None:
                    raise ValueError("This dylib already has an sku_name")
                self.id_cmd = i
