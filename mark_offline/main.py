...
class App:
    ...
    def make_ui(self):
        ...
        self.canvas = Canvas(self.main_panel_frame, width=0, height=0, scrollregion=(0, 0, 0, 0), bg='white',
        ...
        self.position.pack()
        self.test_btn = Button(self.status_bar_frame, text='test', command=self.rotate_img)
        self.test_btn.pack()
    ...
    def mouse_move(self, event):
        ...
        if self.is_cursor_select:
            ...
            self.position.configure(text='%d : %d' % (x, y))
            if self.h_line:
                self.canvas.delete(self.h_line)
            self.h_line = self.canvas.create_line(self.canvas.canvasx(0), event.y, self.canvas.canvasx(self.canvas.winfo_width()), event.y, width=1)
            if self.v_line:
                self.canvas.delete(self.v_line)
            self.v_line = self.canvas.create_line(event.x, self.canvas.canvasy(0), event.x, self.canvas.canvasy(self.canvas.winfo_height()), width=1)
    ...
    def zoom_img(self, event):
        ...
        print('%d%%' % int(self.cur_zoom_level * 100), self.main_panel_frame.winfo_height() - zoom_height, self.img.size)
        # preview_box zoom
        self.make_preview_box(event)
        # bbox zoom
        # for bbox in self.bbox_list:
            ...
            with open(annotation_path, 'r', encoding='UTF-8') as f:
                for i, line in enumerate(f):
                    ...
                    if i == 0:
                        ...
                        self.cur_img_rotate = int(line.split()[-1])
                        continue
                    strs = line.split()
                    category = strs[0]
    ...
    def open_img(self, img_path):
        self.img = Image.open(img_path)#.rotate(-45,expand=TRUE)
        print(img_path)
        self.img_size = self.img.size
        self.tk_img = ImageTk.PhotoImage(self.img)
        # self.canvas.config(scrollregion=(self.canvas.canvasx(0), self.canvas.canvasy(0), self.canvas.canvasx(self.img_size[0]), self.canvas.canvasy(self.img_size[1])))
        self.canvas.config(scrollregion=(self.canvas.canvasx(0), self.canvas.canvasy(0), self.canvas.canvasx(self.img_size[0]), self.canvas.canvasy(self.img_size[1])))
        self.canvas.delete('all')
        self.cur_img_id = self.canvas.create_image(self.canvas.canvasx(0), self.canvas.canvasy(0), image=self.tk_img, anchor=N+W)
    def rotate_img(self,event=None):
        if event:
            if event.delta > 0:
                self.cur_img_rotate+=5
            else:
                self.cur_img_rotate-=5
