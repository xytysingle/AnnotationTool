...
def parse_args():
    ...
    parser.add_argument('--gpu_fraction', help='GPU amount to use', default=0.3, type=float)
    ...
    try:
        while True:
            ...
            if not isinstance(im_keys, list):
                ...
                continue
            if len(im_keys) != 3:
                logger.error('im_keys not contain 3 elements.')
                continue

            logger.info('Processing {}, {} and {}...'.format(im_keys[0], im_keys[1], im_keys[2]))

            try:
                _im_file1 = image_store_cos.get_as_file(im_keys[0])
                _im_file2 = image_store_cos.get_as_file(im_keys[1])
                _im_file3 = image_store_cos.get_as_file(im_keys[2])
            except Exception:
            ...
            if image1 is None or image2 is None or image3 is None:
                ...
                print(class2id[label_map[int(pred)]])
                try:
                    result = {
                        'image': image_oris,
                        'context': context,
