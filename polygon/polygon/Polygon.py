...
class Main(BaseApp):
    ...
    def bind_event(self):
        ...
        self.master.bind_all("<KeyPress-BackSpace>", self.del_point)
    ...
    def mouse_click(self, event,isDouble=False):
        if self.is_cursor_select:
            ...
            if self.init_dot['zoom_level'] != None:
                ...
                # self.canvas.create_line(self.point_list[-1][0], self.point_list[-1][1], event.x, event.y,
                                        # width=self.bd_width,
                                        # fill=self.getObjByCategory(
                ...
                if len(self.point_list)<3 :
                    ...
                    if event.x > xMin and event.x < xMax and event.y > yMin and event.y < yMax or isDouble:
                        ...
                        if self.is_change_coord:
                            ...
                            rectangle_id = self.canvas.create_polygon(point_list,
                                                                        width=self.bd_width,
                                                                        outline=self.getObjByCategory(bbox.className).color,stipple=self.is_stipple,fill=bbox.color,
                ...
                x1, y1 = (event.x - self.init_dot['radius']), (event.y - self.init_dot['radius'])
                x2, y2 = (event.x + self.init_dot['radius']), (event.y + self.init_dot['radius'])
                self.init_dot['point'] = [x1, y1, x2, y2]
    ...
    def mouse_move(self, event):
        ...
        if   len(curselection) or self.is_change_coord:
            ...
            self.is_cursor_select=True
            self.toggle_cursor()
        # if self.point_list and curselection:
        #     curselection_category = self.categories[curselection[0]]
        #     self.canvas.delete('preview_line')
        #     self.canvas.create_line(self.point_list[-1][0], self.point_list[-1][1], event.x, event.y,
        #                             width=self.bd_width,
        #                             # outline=self.getObjByCategory(curselection_category).color,
        #                             fill=self.getObjByCategory(curselection_category).color,
        #                             dash=self.truncated, tags=('preview_line',))
    ...
    def make_preview_box(self, event):
        ...
        if self.init_dot['zoom_level'] != None :
            ...
            point_list.append(event.x)
            point_list.append(event.y)
            ...
            if self.is_change_coord:
                ...
                truncated = 1 if type(truncated) == int else ''
                self.temp_rectangle_id = self.canvas.create_polygon(point_list,
                                                                      width=self.bd_width,
                                                                      dash=self.truncated,fill='',
    ...
    def make_menu_bar(self,*args):
        ...
        self.toggleSKUMenu.add_radiobutton(label=self.MENU_VIEW_ITEMS['SKU_MARS'],
                                           command=lambda: self.rdBtn_callback('SKU_MARS'),
    ...
    def get_annotation_data(self,imgBaseName,imgNoWithLocation,is_search):
        ...
        for bbox in self.bbox_list:
            ...
            for i in bbox.box:
                point_list.append(self.getCoordByZoom(i[0]))
                point_list.append(self.getCoordByZoom(i[1]))
            bbox.rectangle_id = self.canvas.create_polygon(point_list,
